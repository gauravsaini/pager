...
2. **Word len <= 5 char**: Every word must be <= 5 chars. Abbrev or clip words > 5 chars (e.g. `service` -> `svc`, `config` -> `cfg`, `database` -> `db`). Standard atomic terms / codes exempt when vital (e.g. `OAuth`, `HTTPS`).
